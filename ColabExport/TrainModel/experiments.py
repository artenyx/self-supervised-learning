...
def ssl_experiment1(model_type, config=None, add_exp_str=''):
    ...
    config['model_type'] = model_type
    model = config['model_type'](config).to(config['device'])
    experiments.run_ssl_experiment(config, 'AE-S-D-'+str(model.__class__.__name__)+add_exp_str, rep_learning_model=model)


def ssl_experiment2(model_type, config=None, add_exp_str=''):
    # SimCLR experiment
    if config is None:
        config = exp_config.get_exp_config()
...
def ssl_experiment6(model_type, config=None, add_exp_str=''):
    # Non denoising single AE experiment ~15 min / 100 usl epochs
    if config is None:
        config = exp_config.get_exp_config()
    config['usl_type'] = 'ae_single'
