...
def save_inp_rec_images(config, epoch, orig_img, img_list, rec_list):
    ...
    save_image(orig_img, '{}/epoch{:03d}_orig.png'.format(config['image_save_path'], epoch))
    for i in range(len(img_list)):
        img = img_list[i].view(orig_img.shape).cpu().data
        rec = rec_list[i].view(orig_img.shape).cpu().data
        save_image(img, '{}/epoch{:03d}_inp{}.png'.format(config['image_save_path'], epoch, i + 1))
        save_image(rec, '{}/epoch{:03d}_rec{}.png'.format(config['image_save_path'], epoch, i + 1))
